	...
	step1 := resource.TestStep{
		...
		Check: resource.ComposeAggregateTestCheckFunc(
			func(s *terraform.State) error {
				assertEqual(t, webhookDataSourceMockGetHandler.Interactions, 2)
			...
			resource.TestCheckResourceAttr("data.fivetran_webhook.test_webhook", "events.0", "sync_start"),
			resource.TestCheckResourceAttr("data.fivetran_webhook.test_webhook", "events.1", "sync_end"),
