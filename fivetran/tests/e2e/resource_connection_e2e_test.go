...
func TestResourceConnectionWithProxyAgentE2E(t *testing.T) {
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() {},
		ProtoV6ProviderFactories: ProtoV6ProviderFactories,
		CheckDestroy:             testFivetranConnectionResourceDestroy,
		Steps: []resource.TestStep{
			{
				...
				resource "fivetran_group" "test_group" {
					provider = fivetran-provider
					name = "test_group_proxy"
			    }

            	resource "fivetran_proxy_agent" "test_proxy_agent" {
                	provider = fivetran-provider
                 	display_name = "test_proxy_agent"
		  ...
		  `,
				Check: resource.ComposeAggregateTestCheckFunc(
					testFivetranConnectionResourceCreate(t, "fivetran_connection.test_connection"),
					resource.TestCheckResourceAttrSet("fivetran_connection.test_connection", "id"),
					resource.TestCheckResourceAttr("fivetran_connection.test_connection", "service", "mysql"),
					resource.TestCheckResourceAttr("fivetran_connection.test_connection", "networking_method", "ProxyAgent"),
					resource.TestCheckResourceAttrSet("fivetran_connection.test_connection", "proxy_agent_id"),
				),
			},
			{
				Config: `
		  ...
		  `,
				Check: resource.ComposeAggregateTestCheckFunc(
					testFivetranConnectionResourceUpdate(t, "fivetran_connection.test_connection"),
					resource.TestCheckResourceAttrSet("fivetran_connection.test_connection", "id"),
					resource.TestCheckResourceAttr("fivetran_connection.test_connection", "networking_method", "Directly"),
					resource.TestCheckNoResourceAttr("fivetran_connection.test_connection", "proxy_agent_id"),
				),
			},
		},
	})
