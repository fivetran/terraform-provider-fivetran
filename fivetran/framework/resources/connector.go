...
func (r *connector) recoverId(ctx context.Context, data model.ConnectorResourceModel) (string, string) {
	...
	if !(data.GroupId.IsNull() || data.GroupId.IsUnknown()) {
		...
		if schemaName != "" && groupId != "" {
			connectorsList, err := r.GetClient().NewGroupListConnections().GroupID(groupId).Limit(1000).Do(ctx)
			found := false
			if err == nil {
				...
				for !found && connectorsList.Data.NextCursor != "" {
					connectorsList, err = r.GetClient().NewGroupListConnections().GroupID(groupId).Limit(1000).Cursor(connectorsList.Data.NextCursor).Do(ctx)
					if err != nil {
						log = log + "\n" + err.Error()
						break
					} else {
