...
func (r *externalLogging) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
    fmt.Printf("==============(r *externalLogging) Update\n")
    if r.GetClient() == nil {
        resp.Diagnostics.AddError(
            "Unconfigured Fivetran Client",
            "Please report this issue to the provider developers.",
    ...
    if !plan.Config.Equal(state.Config) {
        fmt.Printf("==============(r *externalLogging) Update if !plan.Config.Equal(state.Config) {\n")
    	config := plan.GetConfig()
        fmt.Printf("==============(r *externalLogging) Update if !plan.Config.Equal(state.Config) {%v\n", config)
    	svc.ConfigCustom(&config)
    	hasChanges = true
    }

	if hasChanges {
        fmt.Printf("==============(r *externalLogging) Update if hasChanges\n")
    ...
    if runTests && runTests != runTestsState {
        fmt.Printf("==============(r *externalLogging) Update if runTests\n")
