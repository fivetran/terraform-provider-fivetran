...
func (d *ConnectionDatasourceModel) ReadFromResponse(resp connections.DetailsWithCustomConfigNoTestsResponse) {
    responseContainer := ConnectionModelContainer{}
...
func (d *ConnectionResourceModel) ReadFromResponse(resp connections.DetailsWithCustomConfigNoTestsResponse, forceReadConfig bool) diag.Diagnostics {
    responseContainer := ConnectionModelContainer{}
    responseContainer.ReadFromResponseData(resp.Data.DetailsResponseDataCommon, resp.Data.Config)
...
func (d *ConnectionResourceModel) ReadFromCreateResponse(resp connections.DetailsWithCustomConfigResponse) diag.Diagnostics {
    responseContainer := ConnectionModelContainer{}
    responseContainer.ReadFromResponseData(resp.Data.DetailsResponseDataCommon, resp.Data.Config)
...
func (d *ConnectionResourceModel) GetConfigMap(nullOnNull bool) (map[string]interface{}, error) {
...
func (d *ConnectionResourceModel) GetAuthMap(nullOnNull bool) (map[string]interface{}, error) {
...
func (d *ConnectionResourceModel) ReadFromContainer(c ConnectionModelContainer, forceReadConfig bool) {
...
func (d *ConnectionDatasourceModel) ReadFromContainer(c ConnectionModelContainer) {
	d.Id = types.StringValue(c.Id)
	d.Name = types.StringValue(c.Schema)
	d.ConnectedBy = types.StringValue(c.ConnectedBy)
	d.CreatedAt = types.StringValue(c.CreatedAt)
...
type ConnectionModelContainer struct {
	Id          string
	Name        string
	ConnectedBy string
	CreatedAt   string
...
func (c *ConnectionModelContainer) ReadFromResponseData(data connections.DetailsResponseDataCommon, config map[string]interface{}) {
	c.Id = data.ID
	c.Name = data.Schema
...
func getDestinatonSchemaForConfig(serviceId, nameAttr, tableAttr, prefixAttr attr.Value) (map[string]interface{}, error) {
	...
	if _, ok := common.GetDestinationSchemaFields()[service]; !ok {
		return nil, fmt.Errorf("unknown connection service: `%v`", service)
	}
	if common.GetDestinationSchemaFields()[service]["schema_prefix"] {
		if prefixAttr.IsNull() || prefixAttr.IsUnknown() {
			return nil, fmt.Errorf("`destination_schema.prefix` field is required to create `%v` connection", service)
		}
		if !nameAttr.IsNull() {
			return nil, fmt.Errorf("`destination_schema.name` field can't be set for `%v` connection", service)
		}
		if !tableAttr.IsNull() {
			return nil, fmt.Errorf("`destination_schema.table` field can't be set for `%v` connection", service)
		}
		...
		if nameAttr.IsNull() {
			return nil, fmt.Errorf("`destination_schema.name` field is required to create `%v` connection", service)
		}
...
func getDestinationSchemaValuesMap(service, schema string) map[string]attr.Value {
    if _, ok := common.GetDestinationSchemaFields()[service]; !ok {
        panic(fmt.Errorf("unknown connection service: `%v`", service))
    }
