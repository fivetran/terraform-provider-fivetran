...
func (d *Connections) ReadFromResponse(ctx context.Context, resp connections.ConnectionsListResponse) {
	...
	for _, v := range resp.Data.Items {
		...
		item["service_version"] = intPointerStringValue(v.ServiceVersion)
		item["sync_frequency"] = intPointerInt64Value(v.SyncFrequency)
		item["schedule_type"] = types.StringValue(v.ScheduleType)
		item["paused"] = boolPointerValue(v.Paused)
		item["pause_after_trial"] = boolPointerValue(v.PauseAfterTrial)
