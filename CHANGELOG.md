...
### Added
- `fivetran_connector_schema_config`: `create`/`update` timeouts are now configurable via the standard `timeouts` block, bounding the reload/validate/apply sequence (defaults to 30m).

### Fixed
- Connection datasources: handle nullable connection fields returned by the API without crashing the provider.
...
### Fixed
- Resources do better adhere to Terraform provider recommendation - if the resource is missing, the provider should remove it from the state, and return early. The next Terraform plan will recreate the resource.
...
- `fivetran_connection_config` - New attribute `trust_certificates` to automatically trust SSL certificates (defaults to false)
- `fivetran_connection_config` - New attribute `trust_fingerprints` to automatically trust SSH fingerprints (defaults to false)
...
### Fixed
- Allow for importing of `fivetran_private_link` with empty `config_map` (`config_map = {}`).
...
### Added
New connection services supported:
- Supported service: `cube`
- Supported service: `dutchie`
- Supported service: `openai`
- Supported service: `zendesk_workforce_management`

New connection config fields supported:
- Added field `fivetran_connector.config.use_cloud_storage` for services: `amplitude`.
- Added field `fivetran_connector.config.aws_credentials` for services: `amplitude`.
- Added field `fivetran_connector.config.reports.dimension_attributes` for services: `double_click_publishers`.
- Added field `fivetran_connector.config.reports.ad_unit_view` for services: `double_click_publishers`.
- Added field `fivetran_connector.config.project_credentials.project_id` for services: `amplitude`.
- Added field `fivetran_connector.config.reports.columns` for services: `double_click_publishers`.
- Added field `fivetran_connector.config.reports.custom_dimension_key_ids` for services: `double_click_publishers`.
- Added field `fivetran_connector.config.reports.custom_field_ids` for services: `double_click_publishers`.
- Added field `fivetran_connector.config.reports.accounts.flag` for services: `workday_adaptive`.
- Added field `fivetran_connector.config.reports.levels.flag` for services: `workday_adaptive`.
- Added field `fivetran_connector.config.gcs_credentials` for services: `amplitude`.

New destination config fields supported:
- Added field `fivetran_destination.config.s3_bucket_auth_type` for services: `databricks`.
- Added field `fivetran_destination.config.azure_storage_account_auth_type` for services: `databricks`.
- Added field `fivetran_destination.config.external_volume_location` for services: `databricks`.
- Added field `fivetran_destination.config.gcs_service_account_credentials` for services: `snowflake`.
- Added field `fivetran_destination.config.create_external_volumes` for services: `databricks`.
...
### Fixed
- Issue with `fivetran_connector` resource state migration error `AttributeName("destination_schema"): error decoding object; expected 4 attributes, got 3` that occures on provider version upgrade.
...
### Added
Fields
- `host` (String) Private link host.
to `fivetran_private_link` resource, `fivetran_private_link` and `fivetran_private_links` datasource

## [1.9.1](https://github.com/fivetran/terraform-provider-fivetran/compare/v1.9.0...v1.9.1)
...
- New data source `fivetran_connections` instead of deprecated `fivetran_connectors`.
...
### Added
Fields `is_deprecated` and `replacement_role_name` in `fivetran_roles` datasource
...
## Fixed
Issue with default value for `fivetran_connector.hybrid_deployment_agent_id`

## [1.6.1](https://github.com/fivetran/terraform-provider-fivetran/compare/v1.6.0...v1.6.1)

## Fixed
Fix schema import for `fivetran_connector`

## [1.6.0](https://github.com/fivetran/terraform-provider-fivetran/compare/v1.5.1...v1.6.0)

## Removed
Removed deprecated resources and datasources
- Datasources `fivetran_dbt_project`, `fivetran_dbt_projects`, `fivetran_dbt_transformation`, `fivetran_dbt_models`
- Resources `fivetran_dbt_project`, `fivetran_dbt_transformation`, `fivetran_dbt_git_project_config`
...
- Issues with schema migration in `fivetran_dbt_project` resource
